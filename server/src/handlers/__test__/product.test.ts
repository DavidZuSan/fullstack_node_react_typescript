...
describe("GET /api/products url exists", () => {
    ...
    it('GET to JSON response with all products', async () => {
        const response = await request(server)
           .get('/api/products');
...
describe("GET /api/products/:id url exists", () => {
...
describe('PATCH /api/products/:id', () => {
    ...
    it('Should update a product with valid data', async () => {
        ...
        expect(response.status).not.toBe(404)
        expect(response.status).not.toBe(400)
        expect(response.body.error).not.toBe('error')
    });
});
